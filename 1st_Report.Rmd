...
```{r, echo = F}
pacman::p_load(COVID19, tidyverse, tidymodels, workflows, tune, patchwork, lubridate, mice)
theme_set(theme_bw())
...
```{r}
spain$stay_home[278:446] <- "recommended"
```
...
On the one hand, to obtain the number of deaths, confirmed cases and vaccines per week, we will compute firstly the daily cases, substracting today's cases minus tomorrow's and then we will add all the cases of the week. On the other hand, for the categorical variables, we will compute the mode of the variables in every week, i.e., we will give the most frequent values to every week. Below, we can see the distribution of our new variables:
...
b <- spain %>%
  ggplot(aes(x = date, y = confirmed_week)) +
  geom_line()

c <- spain %>%
  ggplot(aes(x = date, y = vaccines_week)) +
  geom_line()
...
1. Even though we will **test** our data with the next weeks, we should divide our training data in two sets: an *analysis* set (with the first 47 weeks), where we will fit our models, and a *validation* set (with the last 16), where we will validate our models in order to finally select the one with the best results. After selecting the best one, we will again fit the model in all our training set (analysis + validation) and then, we will see how it works in our test set.

```{r}
spain_split <- initial_time_split(spain)
spain_analysis <- training(spain_split)
spain_validation <- testing(spain_split)
```

2. Divide the date in month and year and convert it to categorical, because they could be very important for the outcomes.
