...
```{r, echo = F}
pacman::p_load(COVID19, tidyverse, tidymodels, rsample, recipes, parsnip, yardstick, workflows, tune, patchwork, lubridate)
theme_set(theme_bw())
...
3. Even though we will **test** our data with the next weeks, we should divide our training data in two sets: an *analysis* set (with the first 55 weeks), where we will fit our models, and a *validation* set (with the last 8, i.e., around two months), where we will validate our models in order to finally select the one with the best results. After selecting the best one, we will again fit the model in all our training set (analysis + validation) and then, we will see how it works in our test set.
...
spain_rolling <- rolling_origin(
  ...
  assess = 4, # number of weeks used to validate in each resample
  cumulative = F,
  skip = 4
  )
```
...
lm_rec <- 
  ...
  step_rm(date, lag_1_deaths_week) %>%
