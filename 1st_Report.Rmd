...
pacman::p_load(COVID19, tidyverse, tidymodels, rsample, recipes, parsnip, 
               ...
               doParallel, tidyquant, finetune, ggpubr)
...
\bigskip
As we have said in the previous section, we are going to try 5 different models:
\bigskip
...
\bigskip
Thanks to the `workflowsets` package [@workflowsets], we can create two sets of **workflows** (pipelines in python), one for the deaths model and the other for the confirmed cases. A workflow is an object which is formed by a recipe and a model. 
...
workflow_tune_deaths <- workflow_tune_deaths %>%
  filter(wflow_id == c("linear_reg_deaths_lm_wf", "linear_reg_deaths_rlm_wf", "linear_reg_deaths_ridge_wf", "linear_reg_deaths_lasso_wf", "linear_reg_deaths_elastic_net_wf", "pls_deaths_model_wf"))

workflow_results_deaths %>% 
  pull_workflow(id = "linear_reg_deaths_rlm_wf")
workflow_results_deaths %>% 
  pull_workflow(id = "linear_reg_deaths_lm_wf")
...
After defining these models, it is time to tune their parameters. With the R package `doParallel` [@doParallel], we are able to select the number of cores in order to decrease the computational time. In my computer, using 8 cores instead of 4 decreases more than half the computational time, which helps a lot. Next, we modify the intervals of the model parameters, and we will select the set of them that 
minimizes the root mean square error (`rmse`) and the ordinal r-square (`rsq`). Below, we can see the results for the deaths model:
...
# v 2 of 5 tuning:     linear_reg_deaths_ridge_wf (2.8s)
# i 3 of 5 tuning:     linear_reg_deaths_lasso_wf
# v 3 of 5 tuning:     linear_reg_deaths_lasso_wf (2.8s)
# i 4 of 5 tuning:     linear_reg_deaths_elastic_net_wf
# v 4 of 5 tuning:     linear_reg_deaths_elastic_net_wf (1m 12.3s)
# i 5 of 5 tuning:     pls_deaths_model_wf
# v 5 of 5 tuning:     pls_deaths_model_wf (1m 18.9s)
...
# v 1 of 5 resampling: linear_reg_confirmed_lm_wf (9.3s)
# i 2 of 5 tuning:     linear_reg_confirmed_ridge_wf
# v 2 of 5 tuning:     linear_reg_confirmed_ridge_wf (2.8s)
...
# v 4 of 5 tuning:     linear_reg_confirmed_elastic_net_wf (1m 3.2s)
# i 5 of 5 tuning:     pls_confirmed_model_wf
# v 5 of 5 tuning:     pls_confirmed_model_wf (1m 8.6s)
...
en_param <- elastic_net_model %>%
  parameters() %>%
  update(penalty = penalty(c(2.652128e-05-1e-5, 2.652128e-05+1e-5)),
         mixture = mixture(c(0.9843659-0.01, 0.9843659+0.01)))
...
pls_param <- pls_model %>%
  parameters() %>%
  update(predictor_prop = predictor_prop(c(0.1088969-0.05, 0.1088969+0.05)),
         num_comp = num_comp(c(3-2, 3+2)))
...
# i	No tuning parameters. `fit_resamples()` will be attempted
# i 1 of 5 resampling: linear_reg_deaths_lm_wf
# v 1 of 5 resampling: linear_reg_deaths_lm_wf (9.9s)
# i 2 of 5 tuning:     linear_reg_deaths_ridge_wf
# v 2 of 5 tuning:     linear_reg_deaths_ridge_wf (2.7s)
# i 3 of 5 tuning:     linear_reg_deaths_lasso_wf
# v 3 of 5 tuning:     linear_reg_deaths_lasso_wf (2.6s)
# i 4 of 5 tuning:     linear_reg_deaths_elastic_net_wf
# v 4 of 5 tuning:     linear_reg_deaths_elastic_net_wf (36.6s)
# i 5 of 5 tuning:     pls_deaths_model_wf
# v 5 of 5 tuning:     pls_deaths_model_wf (37.9s)
...
# v 1 of 5 resampling: linear_reg_confirmed_lm_wf (9.3s)
# i 2 of 5 tuning:     linear_reg_confirmed_ridge_wf
# v 2 of 5 tuning:     linear_reg_confirmed_ridge_wf (2.6s)
# i 3 of 5 tuning:     linear_reg_confirmed_lasso_wf
# v 3 of 5 tuning:     linear_reg_confirmed_lasso_wf (2.4s)
# i 4 of 5 tuning:     linear_reg_confirmed_elastic_net_wf
# v 4 of 5 tuning:     linear_reg_confirmed_elastic_net_wf (34.2s)
# i 5 of 5 tuning:     pls_confirmed_model_wf
# v 5 of 5 tuning:     pls_confirmed_model_wf (39.7s)
...
```{r}
workflow_results_deaths <- readRDS("workflow_results_deaths.RDS")
workflow_results_deaths_tuned <- readRDS("workflow_results_deaths_tuned.RDS")
workflow_results_confirmed <- readRDS("workflow_results_confirmed.RDS")
workflow_results_confirmed_tuned <- readRDS("workflow_results_confirmed_tuned.RDS")
```
...
As we can observe above, there is a big difference between the partial least squares model and the rest. In the same way, as we can see for the models of confirmed cases, the one with the best metrics is the partial least squares, but for the next one, for instance, although the lasso model is the last one in `rmse`, it is the second in `rsq`
...
stronger the association. For the deaths model, we can observe two main groups that
are very correlated with the outcome, however, for the confirmed model, some variables
appear in the center of the model, i.e., they are not so correlated. 

```{r, fig.cap="Correlation Circle Plot for the deaths model"}
...
p2 <- spain %>%
  ...
  select(date, confirmed_week, predictions) %>%
...
spain_new_data %>%
  ...
  select(date, deaths_week, predicted_deaths, confirmed_week, predicted_confirmed) %>%
  summarise(
    date = date,
    deaths_week = deaths_week, 
    predicted_deaths = predicted_deaths,
    difference_deaths = abs(deaths_week - predicted_deaths),
    confirmed_week = confirmed_week, 
    predicted_confirmed = predicted_confirmed,
    difference_confirmed = abs(confirmed_week - predicted_confirmed)
  )
```
