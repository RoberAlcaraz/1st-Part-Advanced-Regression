...
# saveRDS(spain, "../00_data/spain_clean.RDS")
```
...
our training data into folds to tune the model parameters and to compare the model results. To
...
steps that the models need. For the linear regression models (`lm`, `rlm`, `ridge`, `lasso` and `elastic net`), the recommended preprocessing is to remove the zero variance variables, decorrelate the predictors and create dummy variables for the categorical predictors; and for the partial least squares (PLS), we need to normalize the predictors, i.e., transform them to have mean equal to 0 and variance equal to 1.
...
```{r, fig.width=8, fig.cap="Correlation Circle Plot for the deaths model"}
...
```{r, fig.width=8, fig.cap="Results in the training set of both models: deaths (left) and confirmed cases (right)"}
# saveRDS(spain, "../00_data/spain_weekly.RDS")
...
# saveRDS(results, "../02_results/results.RDS")
knitr::kable(results, caption = "Results in the testing set")
```

As we can observe, our model for predicting the deaths is not doing a great job,
since it is predicting more than double the observed deaths, so we must think about
maybe computing one different model for each time step, or doing a recursive time
series model, i.e., using the prediction of one day to predict the following.

On the other hand, the model for predicting the confirmed cases in Spain has very
good results. The difference between the observed and the predicted values is around
6000 confirmed cases, which is only an error of around 30%. Besides, if we consider
the sum of these three weeks, there has been 63667 new confirmed cases in Spain and
our model has predicted 68997, so we have only a difference of 5000 cases in a period
of three weeks, so it is a great results.

In a conclusion, we have realized that only with 60 weeks is very difficult to obtain
great models and, even though the second model is very good, both can be improved.
In the following step of the project, we will see other strategies to improve them
as well as new more powerful machine learning models to get better results.
